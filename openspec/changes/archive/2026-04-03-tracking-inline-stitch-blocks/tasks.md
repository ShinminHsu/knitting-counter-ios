...
- [x] 5.2 [手動驗證] Push 到 GitHub，在另一台電腦執行 `npx expo run:ios`，建立含群組 × 2 + 單針 + 多針法的圈，確認：
  - [x] 5.2.1 [手動] 所有 block 流式排列，群組 label 與後續元素同行
  - [x] 5.2.2 [手動] 點群組 label → 整個重複完成
  - [x] 5.2.3 [手動] 點群組內單個符號 → 只推進一針
  - [x] 5.2.4 [手動] 點多針法 label（e.g. `hdc × 5`）→ 全部完成
  - [x] 5.2.5 [手動] 點多針法內單個符號 → 只推進一針
  - [x] 5.2.6 [手動] 符號顏色正確：已完成淺灰、當前粉紅、未完成深灰
  - [x] 5.2.7 [手動] icon / text 模式切換後仍正常；預覽模式點擊無效
